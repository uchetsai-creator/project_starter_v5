# Document Purposes — Scripts & Tooling Reference

<!--
  Reference only. Not read every task.
  Split out of document-purposes-common.md so that file stays focused on document
  purposes (what each docs/ file is for) without every session also paying for a full
  script/CLI-flag reference. Load this file only when you actually need to look up a
  script's flags or behavior -- e.g. writing or debugging a workflow-registry.yaml
  entry, wiring a new validator into pre-commit, or checking exactly what a generator
  script does. Most tasks never need this file.
-->
...
and Codex today. Optional Claude Code Stop hook (`.claude/settings.json`) calls the same scripts for

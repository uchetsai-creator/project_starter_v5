...
description: Use only when a module has just been confirmed 100% complete (all its tasks are marked done in docs/project-plan.md) — inserts logger calls per logging-spec.md, offers optional debug instrumentation, and decides whether to rebuild the PDF. Do NOT use mid-module, and do NOT use for ordinary per-task closeout (see task-closeout instead).
---
...
* Does completing this task finish all work for its module in docs/project-plan.md?
  ...
  * If yes: this module is now complete. Do all of the following:
    ...
    4. Rebuild the PDF only if ANY of the following conditions are met:
       ...
       Note: to add a new doc to the PDF, add it to docs/script/generators/pdf_allowlist.py only —

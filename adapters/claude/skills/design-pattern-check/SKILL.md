...
description: Opt-in deep design-pattern audit (all 23 GoF patterns + Registry) — checks both missing-pattern opportunities (duplicated type-dispatch logic that should collapse into Strategy/Factory/Registry/Template Method/etc.) and pattern misuse (over-engineered abstractions with no real second implementation). Two modes: Fast (static-only, reads and cross-compares current code, catches live/already-manifest issues) and Deep (adds git-history mining to also catch predictable-future-risk cases). NOT run automatically as part of code-quality-check. Only run when the user explicitly asks for a design pattern review, or after code-quality-check asks and the user confirms they want it included and picks a mode.
---

# AI Context — ml-pipeline / pipeline-stage
Generated: {{TIMESTAMP}}
...
- docs/specs/model-contract.md   # Input feature schema, output format, and production score thresholds
    update when: input feature schema, output format, or threshold changed; model version, training data requirements, or retraining policy changed
...
- docs/specs/experiment-log.md   # Per-experiment results log (one row per training run)
    update when: new training run completed

...
Extracts NormalizedStageContract objects from:
  ...
  - Code: Python files — @op / @asset decorated functions with type-annotated I/O
...
def _is_dagster_decorator(dec) -> bool:
    if isinstance(dec, ast.Name):
        return dec.id in _DAGSTER_DECORATORS
    if isinstance(dec, ast.Attribute):
        return dec.attr in _DAGSTER_DECORATORS
...
class DagsterAdapter(FrameworkAdapter):
    ...
    Code format (Python):
      @op
      def stage_name(context, field: type) -> OutputType:
          ...

      @asset
      def stage_name(upstream: type) -> OutputType:
          ...
    """

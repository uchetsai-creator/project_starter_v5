"""
prefect.py — PrefectDetector (+ legacy PrefectAdapter) for project_starter_v5.

Phase 52.5: PrefectDetector is the primary class — it receives pre-discovered
files from DataPipelineAdapter and returns NormalizedStageContract objects.
PrefectAdapter is kept as a backward-compatible shim.

Extracts NormalizedStageContract objects from:
  - Spec: pipeline-contract.md (### Stage sections with #### Input/Output Contract tables)
  - Code: Python files — @task / @flow decorated functions with type-annotated I/O
...
def _is_prefect_decorator(dec) -> bool:
    if isinstance(dec, ast.Name):
        return dec.id in _PREFECT_DECORATORS
    if isinstance(dec, ast.Attribute):
        return dec.attr in _PREFECT_DECORATORS
    if isinstance(dec, ast.Call):
        return _is_prefect_decorator(dec.func)
    return False


class PrefectDetector(Detector):
    """
    Framework detector for Prefect (Phase 52.5).

    Receives pre-discovered .py files from DataPipelineAdapter.
    Returns NormalizedStageContract for each @task/@flow decorated function.
    ...
    def _parse_file(self, fpath: str) -> list[NormalizedStageContract]:
        ...
        for node in ast.walk(tree):
            if not isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                continue
            if not any(_is_prefect_decorator(d) for d in node.decorator_list):
                continue
...
class PrefectAdapter(FrameworkAdapter):
    ...
    Adapter for Prefect (Python) Data Pipeline / ML Pipeline projects.
    ...
    Code format (Python):
      ...
      @flow
      def pipeline(field: type) -> OutputType:
          ...
    """
    ...
    def _parse_file(self, fpath: str) -> list[NormalizedStageContract]:
        return PrefectDetector()._parse_file(fpath)
